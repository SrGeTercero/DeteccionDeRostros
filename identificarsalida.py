...
def insertorupdate(ID,carnet):
    conn=sqlite3.connect("Fasebase.db")
    cmd="SELECT * FROM asistenciasalida WHERE id="+str(ID)
    ...
    for row in cursor:
        isRecordExist=1
    if(isRecordExist==1):
        #cmd="UPDATE asistenciasalida SET carnet='"+str(carnet)+"' WHERE id="+str(ID)
        cmd="UPDATE asistenciasalida SET carnet='"+str(carnet)+"', fecha='"+str(ahora)+"' WHERE id="+str(ID)
    else:
        #cmd="INSERT INTO asistencia (id, carnet) VALUES ("+str(ID)+", '"+str(carnet)+"')"
        cmd="INSERT INTO asistenciasalida (id, carnet, fecha) VALUES ("+str(ID)+", '"+str(carnet)+"', '"+str(ahora)+"')"
    conn.execute(cmd)
    conn.commit()
    conn.close()
